					...
					callbacks: {
						...
						label: (context) => {
							if (context.datasetIndex === 0) {
								return `上行: ${context.parsed.y.toFixed(2)} KB/s`;
							}
							return `下行: ${context.parsed.y.toFixed(2)} KB/s`;
						},
...
const updateChartRealtime = () => {
	...
	() => realtimeData['timestamp'], () => {
		...
		chartInstance.data.datasets[0].data.push(realtimeData['net_upload']);
		chartInstance.data.datasets[1].data.shift();
		chartInstance.data.datasets[1].data.push(realtimeData['net_download']);
		chartInstance.update();
	});
};
...
const updateChartHistorical = async () => {
	...
	const [netUpload, netDownload] = await Promise.all([
		fetchHistoricalData('net_upload', currentTimeFrame),
		fetchHistoricalData('net_download', currentTimeFrame),
	]);
	chartInstance.data.labels = netUpload['labels'];
	chartInstance.data.datasets[0].data = netUpload['data'];
	chartInstance.data.datasets[1].data = netDownload['data'];
